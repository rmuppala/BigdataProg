...
object Utils {
  ...
  private def noOfWords(s: String): String =
    ", Total no of Words : " + s.split(' ').size.toString
  ...
  def makeReadable(sn: (String, Int)): String =
    sn match {
      case (tweetText, score) => s"${wrapScore(makeReadable(score))}${makeReadable(tweetText)}${noOfWords(makeReadable(tweetText))}"
    }
